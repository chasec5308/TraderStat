...
.stat-value.orange { color: #F6AD55; }
...
# ── Quick Stats ────────────────────────────────────────────────────────────────
# Fetch data first — df is only created inside the `if trades:` block below.
trades   = db.get_trades()
accounts = db.get_prop_accounts()

# ── Helper: compute Trader Score ───────────────────────────────────────────────
def compute_trader_score(stats: dict) -> int:
    """
    Score a trader 0-100 based on four equally-weighted components (25 pts each):
      1. Win Rate       — 50% = 12.5 pts, 60% = 25 pts (capped)
      2. Profit Factor  — 1.0 = 0 pts, 2.0+ = 25 pts (linear, capped)
      3. Avg R          — 0 = 0 pts, 1.5R+ = 25 pts (linear, capped)
      4. Expectancy     — negative = 0 pts, $200+/trade = 25 pts (linear, capped)
    Returns an integer between 0 and 100.
    """
    # Component 1: Win Rate (50% baseline → 25 pts at 60%+)
    wr = stats.get("win_rate", 0)
    wr_score = min(25, max(0, (wr - 30) / (60 - 30) * 25))

    # Component 2: Profit Factor (1.0 → 0 pts, 2.0+ → 25 pts)
    pf = stats.get("profit_factor", 0)
    if pf == float("inf"):
        pf_score = 25.0
    else:
        pf_score = min(25, max(0, (pf - 1.0) / (2.0 - 1.0) * 25))

    # Component 3: Average R (0R → 0 pts, 1.5R+ → 25 pts)
    avg_r = stats.get("avg_r", 0)
    r_score = min(25, max(0, avg_r / 1.5 * 25))

    # Component 4: Expectancy (0 → 0 pts, $200+ → 25 pts)
    exp = stats.get("expectancy", 0)
    exp_score = min(25, max(0, exp / 200 * 25))

    total = wr_score + pf_score + r_score + exp_score
    return int(round(total))


# ── Render the 5 stat cards ────────────────────────────────────────────────────
col1, col2, col3, col4, col5 = st.columns(5)

if trades:
    stats = utils.compute_stats(trades)
    trader_score = compute_trader_score(stats)

    # Colour for Trader Score
    if trader_score >= 70:
        score_cls = "green"
    elif trader_score >= 40:
        score_cls = "orange"
    else:
        score_cls = "red"
    ...
    with col2:
        wr_cls = "green" if stats["win_rate"] >= 50 else "red"
        st.markdown(f"""
        <div class="stat-card">
            <div class="stat-label">Win Rate</div>
            <div class="stat-value {wr_cls}">{stats['win_rate']}%</div>
        </div>""", unsafe_allow_html=True)

    with col3:
        pnl_cls = "green" if stats["total_pnl"] >= 0 else "red"
        st.markdown(f"""
        <div class="stat-card">
            <div class="stat-label">Total P&L</div>
            <div class="stat-value {pnl_cls}">${stats['total_pnl']:,.2f}</div>
        </div>""", unsafe_allow_html=True)

    with col4:
        st.markdown(f"""
        <div class="stat-card">
            <div class="stat-label">Prop Accounts</div>
    ...
    with col5:
        ...
        <div class="stat-card">
            ...
            <div class="stat-value {score_cls}">{trader_score}<span style="font-size:1rem;color:#A0AEC0">/100</span></div>
        </div>""", unsafe_allow_html=True)

else:
    # No trades yet — show placeholder values for all 5 cards
    placeholders = [
        ("Total Trades",  "0",   ""),
        ("Win Rate",      "—",   ""),
        ("Total P&L",     "$0.00", ""),
        ("Prop Accounts", str(len(accounts)), ""),
        ("Trader Score",  "0<span style=\"font-size:1rem;color:#A0AEC0\">/100</span>", ""),
    ]
    for col, (label, val, cls) in zip([col1, col2, col3, col4, col5], placeholders):
        with col:
            st.markdown(f"""
            <div class="stat-card">
...
# ── Recent Trades ──────────────────────────────────────────────────────────────
# df is only created here, safely inside the `if trades:` guard.
if trades:
...
# ── Getting Started (shown only when no trades exist) ──────────────────────────
if not trades:
    st.markdown('<div class="section-header">Getting Started</div>', unsafe_allow_html=True)
    steps = [
        ("1", "📒", "Log Your First Trade",
         "Go to Trade Journal and log your first trade. The app will automatically calculate your R multiple and P&L."),
        ("2", "🧮", "Calculate Position Size",
         "Use the Risk Calculator before every trade to ensure you never risk more than your defined percentage."),
        ("3", "🏦", "Track Your Prop Evaluation",
         "Add your prop firm account to monitor your drawdown and progress toward the profit target."),
    ]
    c1, c2, c3 = st.columns(3)
